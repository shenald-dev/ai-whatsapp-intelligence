...
def process_message(message_id: str):
    ...
    try:
        # Fetching only required columns via load_only to minimize database bandwidth and memory consumption,
        # especially since the Message table can contain large text payloads (e.g., content, quoted_msg_id).
        # Note: If future logic requires additional fields, they must be added to this load_only list
        # to prevent inefficient lazy-loading network calls.
