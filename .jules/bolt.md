...
## 2024-05-18 — Optimize SQLAlchemy Query Efficiency

Learning:
When only a subset of fields is required from a SQLAlchemy model (especially those containing large `Text` columns), fetching the entire object over the network wastes bandwidth and memory.

Action:
Use the `load_only` option with `session.get()` (e.g., `session.get(Model, id, options=[load_only(Model.col1, Model.col2)])`) to fetch only necessary columns. This optimizes database bandwidth and memory while safely preserving the ORM model contract.
