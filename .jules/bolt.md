...
## 2026-05-06 — Direct SQL UPDATE in background worker hot path

Learning:
When updating a subset of fields on a SQLAlchemy model containing large columns (e.g., `Text` fields up to 64KB) within a hot path (like Celery workers), preferring a direct SQL `UPDATE` statement via `session.execute(update(Model).where(...).values(...))` instead of `session.get()` followed by attribute assignment significantly reduces network bandwidth usage and memory overhead. Using `session.get()` on a fresh transaction unnecessarily fetches the entire model over the network, including the potentially massive text payloads, just to update a few small analysis flags.

Action:
Refactored `process_message` in `backend/app/workers/tasks.py` to use direct `UPDATE` statements for modifying the `sentiment` and `classification` attributes. This ensures faster, lower-memory task execution without querying heavy data unnecessarily.
...
## 2026-05-15 — Optimize Celery Worker Data Retrieval

Learning:
When retrieving objects from the database inside Celery tasks (like `process_message` in `backend/app/workers/tasks.py`), fetching the entire object (e.g., using `session.get(Message, message_id)`) retrieves all columns. If the object contains large data, or if only a few specific columns are needed, fetching all columns consumes unnecessary database bandwidth and memory.

Action:
Used `load_only` within `session.get` options to only fetch the explicitly required columns (`Message.is_analyzed`, `Message.content`, `Message.group_id`, `Message.sender_id`) instead of the entire row. This significantly reduces the size of the retrieved payload, improving memory utilization and database network efficiency.
